...
struct TestActorNetwork: Network {
    ...
    func prediction(_ input: AgentInput<TestEnvironment.Observation, Int>) async throws -> ActorOutput<TestEnvironment.ActionDistribution, Int> {
    ...
    func update(
        ...
        with lossFunc: (ActorOutput<TestEnvironment.ActionDistribution, Int>) async throws -> ActorOutput<TestEnvironment.ActionDistribution, Int>
    ) async throws -> Loss {
        
        let output = try await self.prediction(input)
        let target: TestActorNetwork.Output = try await lossFunc(output)
        
        let buffer1: [Float] = [input.observation.fitness]
        let buffer2: [Float] = [input.observation.fatigue]
        let buffer3: [[Float]] = [target.actionDistribution.actionDistribution.probabilities]
