...
struct TestQNetwork: Network {
    ...
    func prediction(_ input: AgentInput<TestEnvironment.Observation, Int>) async throws -> QNetworkOutput<Int> {
        ...
        return try await withCheckedThrowingContinuation { continuation in
            self.inferenceGraph.execute(
                ...
                let output = QNetworkOutput(
                    qValues: Array(float4Buffer),
                    state: input.state
                )
    ...
    func update(
        ...
        with lossFunc: (QNetworkOutput<Int>) async throws -> QNetworkOutput<Int>
    ) async throws -> Loss {
        
        let output = try await self.prediction(input)
        let target = try await lossFunc(output)
        
        let buffer1: [Float] = [input.observation.fitness]
        let buffer2: [Float] = [input.observation.fatigue]
        let buffer3: [[Float]] = [target.qValues]
